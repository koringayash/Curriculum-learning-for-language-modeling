...
Preserved exactly from original project.
"""
import math
import torch
...
def apply_rope(q: torch.Tensor, k: torch.Tensor, freqs_cis: torch.Tensor) -> tuple:
    ...
    k_c = torch.view_as_complex(k.float().reshape(*q.shape[:-1], -1, 2))
    f = freqs_cis[:T].unsqueeze(0).unsqueeze(0)
    return torch.view_as_real(q_c * f).flatten(3).type_as(q), torch.view_as_real(k_c * f).flatten(3).type_as(k)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:
        ...
        def to_heads(t): return t.view(B, T, self.n_head, self.head_dim).transpose(1, 2)
...
class SwiGLU(nn.Module):
    def __init__(self, n_embd: int, dropout: float):
        ...
        self.w1, self.w2, self.w3 = nn.Linear(n_embd, hidden, bias=False), nn.Linear(hidden, n_embd, bias=False), nn.Linear(n_embd, hidden, bias=False)
...
class TransformerBlock(nn.Module):
    def __init__(self, n_embd: int, n_head: int, dropout: float):
        super().__init__()
        self.norm1, self.attn = RMSNorm(n_embd), CausalSelfAttention(n_embd, n_head, dropout)
        self.norm2, self.ffn = RMSNorm(n_embd), SwiGLU(n_embd, dropout)
    def forward(self, x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:
...
class GPT(nn.Module):
    ...
    def _init_weights(self, module):
        if isinstance(module, (nn.Linear, nn.Embedding)): nn.init.normal_(module.weight, std=0.02)
    ...
    def forward(self, idx: torch.Tensor, targets: torch.Tensor = None) -> tuple:
        ...
        for block in self.blocks: x = block(x, self.freqs_cis[:T])
